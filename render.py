...
Purp = {"type": "Diffuse", "brdf": np.array([0.75, 0.6, 0.75]), "isdelta": False}
def reflect(in_dir, normal):
    return 2. * np.dot(in_dir, normal) * normal - in_dir
def cosWeightHemiSPhere(in_dir, n):
    ...
    return np.matmul(np.transpose(np.array([v, np.cross(v, n), n])), np.array([np.sin(theta) * sin_p, np.cos(theta) * sin_p, cos_p])), cos_p
Sampler = {"Diffuse": cosWeightHemiSPhere, "Mirror": reflect}
class Ray:
...
class Plane: # A hard code plane, center can only be (x,0,0), (0,y,0), (0,0,z)
    def __init__(self, length, center, normal, axis, material):  # which axises are not zero
        self.length, self.center, self.normal, self.zeroaxis, self.index, self.material = length, center, normal, axis, {0, 1, 2}.difference(set(axis)).pop(), material
...
my_scene = [Sphere(np.array([-28., 15., 20.]), Diffuse, 20), Plane(50, np.array([50, 0, 0]), np.array([-1., 0., 0.]), [1, 2], Diffuse),  # Celling
            Plane(50, np.array([0, 50, 0]), np.array([0., -1., 0.]), [0, 2], Red), Plane(50, np.array([0, -50, 0]), np.array([0., 1., 0.]), [0, 2], Blue), Plane(50, np.array([-50, 0, 0]), np.array([1., 0., 0.]), [1, 2], Diffuse), Plane(50, np.array([0, 0, 50]), np.array([0., 0., -1.]), [0, 1], Diffuse)]
sample_num, max_iter = 1, 2
